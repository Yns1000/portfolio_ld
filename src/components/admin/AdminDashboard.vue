    ...
    <main class="main-stage">
      ...
      <div class="stage-content custom-scrollbar">
        ...
        <div v-if="viewMode === 'projects' && currentIdx !== null" class="editor-grid">
          <div class="editor-main">
            <div class="content-card">
              <div class="card-head"><Languages :size="18" class="icon-accent" /> <h3>Contenu éditorial</h3></div>
              <div class="card-body">
          ...
          <aside class="editor-side">
            <div class="content-card">
              <div class="card-head"><Settings :size="18" class="icon-accent" /> <h3>Paramètres</h3></div>
              <div class="card-body">
                <div class="input-box">
                ...
                <div class="input-box"><label>Lien étude de cas</label><input v-model="projects[currentIdx].link" /></div>
                <div class="input-box"><label>Image de couverture (URL)</label><input v-model="projects[currentIdx].image" /></div>
              </div>
            </div>
          </aside>
        </div>
        ...
        <div v-else-if="viewMode === 'about'" class="editor-grid">
          <div class="editor-main">
            <div class="content-card">
              <div class="card-head"><User :size="18" class="icon-accent" /> <h3>Biographie détaillée ({{ editLang.toUpperCase() }})</h3></div>
              <div class="card-body">
              ...
              <div class="card-body timeline-manager">
                <div v-for="(item, idx) in aboutData.timeline" :key="idx" class="timeline-edit-item">
                  <div class="timeline-meta-row">
                    ...
                    <div class="input-box type"><label>Catégorie</label>
                    ...
                    <button @click="$emit('delete-timeline', idx)" class="btn-delete-action"><Trash2 :size="16" /></button>
                  </div>
                  <div class="input-box"><label>Intitulé</label><input v-model="item[editLang].title" /></div>
                  <div class="input-box"><label>Lieu / Description courte</label><input v-model="item[editLang].desc" /></div>
                </div>
              </div>
            </div>
            ...
            <div class="content-card" style="margin-top: 24px;">
              ...
              <div class="card-body timeline-manager">
                <div v-for="(cert, idx) in aboutData.certifications" :key="idx" class="timeline-edit-item cert-item">
                  <div class="timeline-row">
                    <div class="input-box" style="flex: 1;"><label>Nom du certificat</label><input v-model="cert[editLang].name" /></div>
                    <button @click="$emit('delete-cert', idx)" class="btn-delete-action" style="margin-top: 24px;"><Trash2 :size="16" /></button>
                  </div>
                  <div class="timeline-row" style="gap: 12px; display: flex;">
                    <div class="input-box" style="flex: 1;"><label>Organisme</label><input v-model="cert[editLang].school" /></div>
          ...
          <aside class="editor-side">
            <div class="content-card">
              ...
              <div class="card-body">
                <div class="input-box"><label>Slogan / Accroche</label><textarea v-model="aboutData[editLang].intro" rows="3"></textarea></div>
              </div>
            </div>
        ...
        <div v-else class="empty-state">
          ...
          <p>Bienvenue Laurine. Sélectionnez une section à gauche pour mettre à jour votre portfolio en temps réel.</p>
        </div>
      </div>
    </main>
  </div>
...
<style scoped>
/* COPIE TON STYLE PRÉCÉDENT ET AJOUTE/MODIFIE CES CLASSES */
...
.timeline-meta-row .type { width: 160px; }

.btn-delete-timeline, .btn-delete-action {
...
.btn-delete-action:hover { background: #ff4757; color: white; }
...
.add-tag-box button { background: #6366f1; border: none; border-radius: 8px; color: white; cursor: pointer; width: 32px; }

/* (Reste du CSS précédent) */
.dashboard-layout { width: 96vw; height: 94vh; max-width: 1600px; background: #070708; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 32px; display: flex; overflow: hidden; box-shadow: 0 50px 100px rgba(0,0,0,0.6); color: #edeeef; font-family: 'Inter', sans-serif; }
...
.app-brand { display: flex; align-items: center; gap: 12px; font-weight: 700; }
...
.nav-tab-btn { display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: transparent; border: none; color: #8a94a6; border-radius: 12px; cursor: pointer; font-weight: 600; transition: 0.2s; }
...
.list-header { padding: 0 12px 12px; display: flex; justify-content: space-between; align-items: center; font-size: 0.7rem; color: #8a94a6; text-transform: uppercase; letter-spacing: 0.1em; }
.btn-plus-sm { background: #6366f1; border: none; border-radius: 6px; color: white; padding: 4px 10px; cursor: pointer; display: flex; align-items: center; gap: 4px; font-size: 0.75rem; font-weight: 700; }
.project-item { padding: 12px; border-radius: 12px; margin-bottom: 4px; cursor: pointer; position: relative; display: flex; align-items: center; transition: 0.2s; }
...
.project-cat { font-size: 0.65rem; color: #8a94a6; text-transform: uppercase; }
...
.btn-save-main { background: #6366f1; color: white; border: none; padding: 14px; border-radius: 12px; font-weight: 700; display: flex; align-items: center; justify-content: center; gap: 10px; cursor: pointer; }
...
.btn-close-app { width: 36px; height: 36px; border-radius: 50%; border: 1px solid rgba(255, 255, 255, 0.1); display: grid; place-items: center; color: #8a94a6; cursor: pointer; }
.btn-close-app:hover { background: rgba(255, 71, 87, 0.1); color: #ff4757; }
...
.input-box label { font-size: 0.7rem; font-weight: 700; color: #8a94a6; text-transform: uppercase; letter-spacing: 0.05em; }
.input-box input, .input-box select, .input-box textarea { background: rgba(0,0,0,0.3); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 10px; padding: 12px; color: white; font-size: 0.9rem; width: 100%; }
.input-box input:focus, .input-box textarea:focus { border-color: #6366f1; outline: none; }
